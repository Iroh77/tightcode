...
| Tool listing policy | `packages/opencode/src/session/tools.ts` (`SessionTools.resolve`), `packages/opencode/src/permission/index.ts` | `ToolListing`: permissible-only filter, eager/deferred shapes (placeholder schema), binding mode, prefix-once MCP rule; kill-switch pass-through |
...
| Binding verdict | `packages/opencode/src/session/` (new resolver module) + XDG app data JSON cache | `BindingVerdict`: static metadata → cached probe → behavioral learning; probe failure → binding |
